...
public class ControladorVideoJuegos implements ManejoDeFechas{
    ...
    //modificamos este metodo en la v2.
    @GetMapping("/getAll")
    ...
    private VideoJuego buscar(Long id){
        VideoJuego v = null;
        for(VideoJuego videojuego : this.videoJuegos){
            if (videojuego.getId() == id) v = videojuego; 
        }
        return v;
    }
    ...
    public String guardar(@RequestParam String nombre,
        ...
        VideoJuego juego = new VideoJuego(nuevoId, nombre, descripcion, portada, formatearFecha(fecha_lanzamiento), Double.parseDouble(rating));
